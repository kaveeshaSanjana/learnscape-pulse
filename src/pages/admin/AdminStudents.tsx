      ...
      render: (s) => (
        <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            className="font-semibold text-slate-800 text-sm hover:text-blue-600 transition text-left"
            title="View student"
          >
            {s.profile?.fullName || '-'}
          </button>
          <button
            type="button"
            onClick={() => openEdit(s)}
            className="p-1 rounded-md text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition flex-shrink-0"
            title="Edit student"
          >
    ...
    { id: 'joined', label: 'Joined', minWidth: 110, render: (s) => <span className="text-slate-400 text-xs">{s.createdAt ? new Date(s.createdAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : '-'}</span> },
    {
      ...
      render: (s) => (
        <div className="flex items-center justify-end gap-1.5">
          <button onClick={() => setViewStudent(s)} title="View student" className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-slate-50 text-slate-600 hover:bg-slate-100 hover:text-slate-800 transition text-xs font-semibold">
          ...
          <button onClick={() => openEdit(s)} title="Edit student" className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 hover:text-blue-800 transition text-xs font-semibold">
          ...
          <button onClick={() => handleDelete(s.id)} title="Delete student" className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-800 transition text-xs font-semibold">
      ...
      <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] shadow-sm overflow-hidden">
        {loading ? (
          ...
          <StickyDataTable
            ...
            onRowClick={(row) => setViewStudent(row)}
          />
        )}
      </div>
    </div>

...
export default function DashboardPage() {
  ...
  return (
    <div className="w-full space-y-6 animate-fade-in">
      {/* Welcome Hero */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-50 via-sky-50 to-indigo-100 border border-blue-100 p-6 md:p-8">
        <div className="absolute -top-20 -right-20 w-64 h-64 bg-blue-200/40 rounded-full blur-[80px]" />
        <div className="absolute -bottom-20 -left-20 w-48 h-48 bg-indigo-200/30 rounded-full blur-[60px]" />
        <div className="relative z-10 flex items-center justify-between gap-4">
          <div>
            <p className="text-blue-500 text-sm font-medium flex items-center gap-2">
              <span className="text-lg">👋</span> {greeting},
            </p>
            <h1 className="text-2xl md:text-3xl font-bold text-slate-800 mt-1" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{name}!</h1>
            <p className="text-slate-500 text-sm mt-2 max-w-md">Welcome back to Thilina Dhananjaya. Continue where you left off.</p>
            {user?.profile?.instituteId && (
              <div className="inline-flex mt-3 px-3 py-1.5 rounded-lg bg-white/70 border border-blue-200">
                <span className="text-xs text-blue-600 font-mono tracking-wide">ID: {user.profile.instituteId}</span>
              </div>
            )}
          </div>
          <div className="hidden md:flex flex-col items-center gap-2 flex-shrink-0">
            <div className="relative">
              <div className="w-20 h-20 rounded-2xl overflow-hidden border-2 border-blue-200 shadow-lg">
                {avatarUrl ? (
            ...
            <p className="text-slate-400 text-[10px] font-medium tracking-wide uppercase">Online</p>
          </div>
        </div>
      </div>
